...
TEST(TranslationProfile, RejectsNullPromptStrategy) {
    ...
    p.model = LocalModelConfig{std::vector<std::uint8_t>{1, 2, 3}};
...
TEST(TranslationProfile, RejectsEmptyLocalWeights) {
    ...
    auto err = p.validate();
    ASSERT_TRUE(err.has_value());
    EXPECT_NE(err->find("weights are empty"), std::string::npos);
}
