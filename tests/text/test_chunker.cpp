...
TEST(TextChunker, SplitSentencesPreservesEnglishText) {
    ...
    const std::vector<std::string> sentences = qtrans::text::split_sentences(text);
...
TEST(TextChunker, ChunkShortTextReturnsSingleChunk) {
    ...
    const std::vector<std::string> chunks =
        qtrans::text::chunk_by_token_budget(text, 100, char_token_count);
...
TEST(TextChunker, ChunkMergesSentencesUpToBudget) {
    ...
    const std::vector<std::string> chunks =
        qtrans::text::chunk_by_token_budget(text, 10, char_token_count);
...
TEST(TextChunker, ChunkHardSplitsOversizedSentence) {
    ...
    const std::vector<std::string> chunks =
        qtrans::text::chunk_by_token_budget(text, 5, char_token_count);
    ASSERT_GT(chunks.size(), 1u);
    EXPECT_EQ(join_chunks(chunks), text);
    for (const std::string &chunk : chunks) {
...
TEST(TextChunker, ChunkPreservesParagraphBreaks) {
    ...
    const std::vector<std::string> chunks =
        qtrans::text::chunk_by_token_budget(text, 18, char_token_count);
    EXPECT_EQ(join_chunks(chunks), text);
}

TEST(TextChunker, EmptyInputReturnsEmptyChunks) {
    EXPECT_TRUE(qtrans::text::chunk_by_token_budget("", 10, char_token_count).empty());
    EXPECT_TRUE(qtrans::text::split_sentences("").empty());
}

TEST(TextChunker, InvalidBudgetThrows) {
    EXPECT_THROW(
        qtrans::text::chunk_by_token_budget("hello", 0, char_token_count),
